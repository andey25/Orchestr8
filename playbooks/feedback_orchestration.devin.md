...
1. Open https://Orchestr8.fly.dev/rank-issues/pop. This will return to you a json formatted as follows 
    output_json = {
        "title": issue_title,
...
4. Execute the task.
5. If the task is successful, make a pull request on the PR. Additionally, also query the API endpoint by making a post request to https://Orchestr8.fly.dev/success with the following JSON body: { "issue": "[issue link]", "description": "[description of the task you did] }
6. If you encounter any installation issues, pip install those packages.
7. If unsuccessful, make a post request to now make a post request to Orchestr8.fly.dev/failure/ and pass in the issue url, the playbook used, and your reasoning for everything that went wrong.
